...
export function convertUiWorkflow(ui,objectInfo){
  ...
  const graph={},warnings=[];
  for(const node of ui.nodes||[]){
    const id=String(node.id),classType=node.type,info=objectInfo?.[classType];
    if(!info)warnings.push(`Nodo ${id} (${classType}): tipo non trovato in ComfyUI — verifica che il custom node corrispondente sia installato.`);
    const inputs={},linkedNames=new Set();
    (node.inputs||[]).forEach(inp=>{linkedNames.add(inp.name);if(inp.link==null)return;const link=linkById.get(inp.link);if(!link){warnings.push(`Nodo ${id} (${classType}): collegamento mancante per l'input "${inp.name}".`);return}inputs[inp.name]=[String(link[1]),link[2]]});
    const values=Array.isArray(node.widgets_values)?node.widgets_values:[];
    if(values.length){const fields=widgetFields(info,linkedNames);let vi=0;for(const f of fields){if(vi>=values.length)break;inputs[f.name]=values[vi];vi++;if(f.extra)vi++}if(!fields.length&&info)warnings.push(`Nodo ${id} (${classType}): impossibile associare i valori dei widget al nodo.`)}
    if(node.mode===2||node.mode===4)warnings.push(`Nodo ${id} (${classType}): è disattivato/bypassato nel workflow originale — viene comunque incluso ed eseguito normalmente. Disattivalo in ComfyUI se non deve girare.`);
