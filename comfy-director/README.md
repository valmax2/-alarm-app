...
Personaggio + selettore acconciature con anteprime + invio diretto del prompt a un
nodo del workflow) + Fase
...
("Invia al workflow", individuazione strutturale mai indovinata), ma non è ancora
collegato a `prompts.generation_id` né a un percorso "componi → invia → genera" in un
solo click, e lo Smart Prompt Compiler non ha ancora un controllo camera interattivo
